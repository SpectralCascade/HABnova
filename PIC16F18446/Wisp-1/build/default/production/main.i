...
# 4 "K:\\Programs\\MPLABX\\XC8 Compiler\\pic\\include\\c99\\stdlib.h" 2 3
...
# 10 "K:\\Programs\\MPLABX\\XC8 Compiler\\pic\\include\\c99\\stdlib.h" 2 3
...
# 67 "./BME280_driver/bme280_defs.h"
...
_Bool PollGPS()
{
    ...
    _Bool eusart_complete = 0;


    WriteGPS("$PUBX,00*33\r\n");


    while (ticks - startTime < 3000)
    {
        ...
        if (eusart_complete)
        {
            ...
            for (int i = 0, counti = strlen(GetAtRowCSV(nmea_data, PUBX_TIME)); i < counti; i++)
            {
                if (i < 6)
                {
                    ...
                    index = Insert(txdata, csvField[i], index, 140);
                }
            }
            ...
            index = InsertString(txdata, GetAtRowCSV(nmea_data, PUBX_LONG), index, 140);
            index = InsertTxDataSep(index);
...
void main(void)
{
    ...
    while (1)
    {
        ...
        index = InsertString(txdata, "Wisp-1", index, 140);
        index = Insert(txdata, ',', index, 140);
        ClearString(convertedNumber);
        IntToString(id, convertedNumber);
        index = InsertString(txdata, convertedNumber, index, 140);
        index = Insert(txdata, ',', index, 140);
...

        {
            ...
            index = Insert(txdata, '*', end, 140);
            index = InsertString(txdata, checksum, index, 140);
            index = Insert(txdata, '\n', index, 140);
